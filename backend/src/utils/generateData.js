...
const { sequelize, User, Field, Product, Booking, Feedback, Opponent, BookingProduct } = require('../models');
...
async function generateFields(count) {
  logger.info(`Generating ${count} fields...`);
  ...
  logger.info(`${count} fields created`);
}
...
async function generateProducts(count) {
  ...
  while (products.length < count) {
    ...
    const productsOfCategory = categoryProducts[category];
    const product = productsOfCategory[randomInt(0, productsOfCategory.length - 1)];
    ...
    products.push({
      ...
      price: product.price,
      category: category,
      image_url: `https://example.com/products/${product.name.toLowerCase().replace(/\s/g, '_')}_${products.length + 1}.jpg`,
      stock_quantity: category === 'service' ? null : randomInt(10, 100),
      is_available: true
    });
  }
...
async function generateBookings(count) {
  ...
  if (users.length === 0 || fields.length === 0) {
    logger.error('No users or fields found. Cannot generate bookings.');
    return;
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (Math.random() < 0.3 && products.length > 0) {
      ...
      await createdBooking.update({ total_price: totalPrice });
    }
  }
  ...
  logger.info(`${count} bookings created with ${bookingProducts.length} booking products`);
}
...
async function generateFeedback(count) {
  ...
  if (bookings.length === 0) {
    logger.error('No completed and paid bookings found. Cannot generate feedback.');
    return;
  }
  
  // Limit to count or available bookings
  const feedbackCount = Math.min(count, bookings.length);
  logger.info(`Generating ${feedbackCount} feedback entries...`);
  ...
  logger.info(`${feedbackCount} feedback entries created`);
}

async function generateOpponents(count) {
  logger.info(`Generating opponent requests...`);
  ...
  if (bookings.length === 0) {
    logger.error('No confirmed bookings found. Cannot generate opponent requests.');
    return;
  }
  
  // Limit to count or available bookings
  const opponentCount = Math.min(count, Math.floor(bookings.length / 2) * 2);  // Ensure even count for matching
  logger.info(`Generating ${opponentCount} opponent requests...`);
  ...
  for (let i = 0; i < opponentCount; i++) {
    const booking = bookings[i];
    const teamName = `${teamNames[randomInt(0, teamNames.length - 1)]} ${randomInt(1, 99)}`;
    
    opponents.push({
      user_id: booking.user_id,
      booking_id: booking.id,
      team_name: teamName,
      players_count: booking.field_id % 3 === 0 ? 11 : (booking.field_id % 2 === 0 ? 7 : 5),
      skill_level: ['beginner', 'intermediate', 'advanced'][randomInt(0, 2)],
      status: i % 2 === 0 && i < opponentCount - 1 ? 'matched' : 'pending',
      matched_with: i % 2 === 0 && i < opponentCount - 1 ? i + 1 : null,
      contact_phone: generatePhoneNumber(),
      contact_email: `team${i+1}@example.com`,
      notes: Math.random() < 0.5 ? `Looking for a friendly match on ${booking.booking_date}` : null
    });
    
    // Set matched_with for the next opponent if this is matched
    if (i % 2 === 0 && i < opponentCount - 1) {
      opponents[i+1] = {
        user_id: bookings[i+1].user_id,
        booking_id: bookings[i+1].id,
        team_name: `${teamNames[randomInt(0, teamNames.length - 1)]} ${randomInt(1, 99)}`,
        players_count: bookings[i+1].field_id % 3 === 0 ? 11 : (bookings[i+1].field_id % 2 === 0 ? 7 : 5),
        skill_level: ['beginner', 'intermediate', 'advanced'][randomInt(0, 2)],
        status: 'matched',
        matched_with: i,
        contact_phone: generatePhoneNumber(),
      ...
      i++; // Skip the next iteration since we've already created this opponent
    }
  }
  
  await Opponent.bulkCreate(opponents);
  logger.info(`${opponents.length} opponent requests created`);
}
...
async function generateData() {
  try {
    ...
    await generateFields(15);
    await generateProducts(20);
    await generateBookings(200);
    await generateFeedback(100);
    await generateOpponents(50);
